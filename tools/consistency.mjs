...
/* 3. Classe que o JS monta e o CSS nao define (e vice-versa). */
{
  ...
  const orfas = [...definidas].filter((c) => !usadas.has(c) && !c.includes('is-')
    && ![...sufixos].some((s) => c.endsWith(s)) && !/^tuc-(tok|prose|input|btn|menu|badge|check|radio|switch|choice|label|hint|error|tabs|alert|spinner|skeleton|timeline|field|input-group|table|dp|copy)/.test(c));
