var Tucano = (() => {
  ...
  __export(index_exports, {
    ...
    autoInitMarkdown: () => autoInit11,
    autoInitMasks: () => autoInit5,
    autoInitModals: () => autoInit8,
    autoInitRicos: () => autoInit12,
  ...
  function inline(texto) {
    return texto.replace(/`([^`]+)`/g, (_, c) => `<code>${c}</code>`).replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>").replace(/(^|[^*])\*([^*]+)\*/g, "$1<em>$2</em>").replace(/(^|[^_])_([^_]+)_/g, "$1<em>$2</em>").replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (todo, rotulo, url) => {
  ...
  var MARCAS = {
    ...
    codigo: { antes: "`", depois: "`", vazio: "c\xF3digo" },
  ...
  var ATALHOS = { b: "negrito", i: "italico", k: "link" };
  ...
  var Markdown = class {
    constructor(alvo, opcoes = {}) {
      ...
      if (!this.campo) throw new Error("[Markdown] elemento n\xE3o encontrado");
    ...
    _montar() {
      ...
      this.barra = el(
        "div",
        { class: "tuc-md__barra", role: "toolbar", "aria-label": "Formata\xE7\xE3o" },
...
${proximo}`);
    ...
    aplicar(nome) {
      ...
      if (marca.linha) {
        ...
        const novo = trecho.split("\n").map((l) => jaTem ? l.slice(marca.linha.length) : marca.linha + l).join("\n");
      ...
      this._trocar(
        de,
        ate,
        marca.antes + selecionado + marca.depois,
    ...
    html() {
      return renderizar(this.campo.value);
    }
    destroy() {
      this._cleanups.forEach((fn) => fn());
  ...
  function autoInit11(scope = document) {
    ...
    for (const node of scope.querySelectorAll("[data-tuc-markdown]:not([data-tuc-ready])")) {
      ...
      out.push(new Markdown(node, {
        ...
        minRows: node.dataset.rows ? +node.dataset.rows : void 0
      }));
    }
    return out;
  }
  ...
  var ATALHOS2 = { b: "negrito", i: "italico", u: "sublinhado", k: "link" };
  ...
  var Rico = class {
      ...
      this.barra = el(
        ...
        this.opts.toolbar.map((nome) => {
          const b = el("button", {
            type: "button",
            class: "tuc-btn is-ghost is-icon is-sm",
            "aria-label": ROTULOS2[nome] ?? nome,
            "data-tuc-tip": ROTULOS2[nome] ?? nome,
            "aria-pressed": "false",
          ...
          }, [icon(ICONES2[nome] ?? ICONES2.limpar, 15)]);
          b.dataset.acao = nome;
          return b;
        })
      );
    ...
    _teclado(e) {
      ...
      if ((e.metaKey || e.ctrlKey) && ATALHOS2[t]) {
        e.preventDefault();
        this.aplicar(ATALHOS2[t]);
      }
    }
